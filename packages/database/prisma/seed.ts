...
async function main() {
  ...
  const transactions = await prisma.transaction.createMany({
    data: [
      ...
      // Structuring pattern transactions (3 transactions, 8000-9999.99, within 7 days)
      {
        organizationId: org.id,
        customerProfileId: individualCustomer.id,
        transactionImportBatchId: importBatch.id,
      ...
      },
      {
        ...
        customerProfileId: individualCustomer.id,
        transactionImportBatchId: importBatch.id,
        externalReference: "TXN-011",
        direction: TransactionDirection.INBOUND,
        amount: 8800.0,
        currency: "EUR",
        counterpartyName: "Cash Depositor 3",
        counterpartyAccount: "ES91 0000 0000 44 4444444444",
        counterpartyCountry: "ES",
        paymentRail: "CASH",
        transactionType: "DEPOSIT",
        description: "Cash deposit",
        occurredAt: new Date("2024-03-03T10:00:00Z"),
      },
    ],
  });
  ...
  const docs = await prisma.document.createMany({
    data: [
      ...
      },
      {
        organizationId: org.id,
        customerProfileId: individualCustomer.id,
        originalFileName: "maria_garcia_utility_bill.pdf",
        storageKey: "seed/maria_garcia_utility_bill.pdf",
